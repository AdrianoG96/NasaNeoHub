...
export function DateRangeSelector({ onSearch, isLoading }: DateRangeSelectorProps) {
  ...
  const validate = useCallback((start: Date | undefined, end: Date | undefined): string | null => {
    ...
    if (isFuture(start) || isFuture(end)) return "Dates cannot be in the future"

    if (isAfter(start, end)) return "Start date must be before end date"

    const days = differenceInDays(end, start)
    if (days > MAX_RANGE_DAYS) return `Maximum range is ${MAX_RANGE_DAYS} days`
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:gap-3">
      <div className="flex flex-col gap-1.5 sm:flex-1">
        ...
        <Popover open={startOpen} onOpenChange={setStartOpen}>
          <PopoverTrigger render={<Button variant="outline" className={cn("w-full justify-start text-left font-normal border-white/10 bg-white/5 text-white transition-all duration-200 hover:bg-white/10 hover:border-white/20 sm:w-44", !startDate && "text-white/40")} />}>
      ...
      <div className="flex flex-col gap-1.5 sm:flex-1">
        ...
        <Popover open={endOpen} onOpenChange={setEndOpen}>
          <PopoverTrigger render={<Button variant="outline" className={cn("w-full justify-start text-left font-normal border-white/10 bg-white/5 text-white transition-all duration-200 hover:bg-white/10 hover:border-white/20 sm:w-44", !endDate && "text-white/40")} />}>
            <CalendarIcon className="size-4 shrink-0 text-white/50" />
      ...
      <Button
        ...
        className="w-full bg-blue-600 text-white hover:bg-blue-500 transition-all duration-200 active:scale-[0.98] sm:w-auto"
      >
        {isLoading ? (
          <>
